...
int nNodeLayer1 = 10;  // Number of nodes in MLP hidden layer 1
int nNodeLayer2 = 10;  // Number of nodes in MLP hidden layer 2
double wr = 0.0;
double ws = 0.0;
double wu = 0.0;
double sigmaS = 0.0;
...
void printParams () {
  ...
  disp("----------------");
  disp("nNodeLayer1 = %d", nNodeLayer1);
  disp("nNodeLayer2 = %d", nNodeLayer2);
  disp("----------------");
}
    ...
    std::shared_ptr<Input> input, std::string const& inputModelFile)
{
  auto classifier = std::make_shared<Classifier>(
      false, input->dim(), nNodeLayer1, nNodeLayer2);
  if (inputModelFile.empty()) {
    auto gg = random::gaussian(0.0, 0.01, 0);
    int dim = classifier->dim();
    for (int i = 0; i < dim; ++i)
    { classifier->w->operator()(i) = gg(); }
  } else {
...
int main (int argc, char* argv[])
{
  ...
  opts.add_options()
      ...
      ("nn1", bpo::value<int>(&nNodeLayer1),
       "Number of nodes in hidden layer 1 [default: 10]")
      ("nn2", bpo::value<int>(&nNodeLayer2),
       "Number of nodes in hidden layer 2 [default: 10]")
